...
object Vl80sObservationCatalog {
    val observations = listOf(
        ...
        observation("smoke", "Дым, запах горелой изоляции или дуга", ObservationKind.SMELL_OR_HEAT, "Любой такой признак переводит работу в аварийный режим до выяснения причины.", listOf("дым", "горелое", "озон", "дуга", "искрит", "резисторы"), listOf("smoke-fire-flashover", "rheostatic-resistor-overheat", "fire-alarm-signal", "transformer-protection", "protection-trip"), listOf("fire-system", "brake-resistors", "transformer", "vvk")),
    ...
    val equipment = listOf(
        ...
        equipment("brake-lock", "Блокировка тормозов №367М", listOf("БТ 367М", "367", "блокировка тормозов"), "Переключает и блокирует связанные пневматические цепи при смене кабины управления.", listOf("КМ №395", "КВТ №254", "ТМ", "питательная магистраль"), "Кабина машиниста; исполнение и соединения сверять по пневматической схеме.", null, listOf("brake-lock-state-mismatch", "brake-pipe-no-charge", "independent-brake-no-apply")),
        equipment("safety-valve", "Предохранительный клапан главных резервуаров", listOf("предохранительный клапан", "клапан ГР"), "Ограничивает опасное повышение давления в системе главных резервуаров.", listOf("ГР", "компрессор", "АК-11Б", "манометры"), "На пневматической системе главных резервуаров; место зависит от исполнения.", null, listOf("main-reservoir-safety-valve", "compressor-long-run")),
        equipment("aux-contactor", "Контакторы вспомогательных машин", listOf("контактор МВ", "контактор МК", "контакторы собственных нужд"), "Коммутируют питание отдельных вспомогательных двигателей по командам и условиям защиты.", listOf("фазорасщепитель", "двигатели", "релейные панели", "защиты"), "Панели контакторов в аппаратной зоне конкретной секции.", "relay-panels", listOf("aux-machine-single-trip", "aux-common-loss", "aux-contactor-chatter")),
        equipment("ventilation-ducts", "Воздуховоды и заслонки охлаждения", listOf("воздуховод", "заслонка", "тракт охлаждения"), "Распределяют поток мотор-вентиляторов по охлаждаемым группам оборудования.", listOf("мотор-вентиляторы", "ВУ", "ТЭД", "тормозные резисторы"), "В машинном отделении по трассам конкретного исполнения.", null, listOf("ventilation-duct-damage", "fan-low-airflow", "motor-fan-failure", "rheostatic-resistor-overheat")),
        equipment("brake-resistors", "Тормозные резисторы", listOf("резисторы реостатного торможения", "тормозные сопротивления"), "Рассеивают электрическую энергию в режиме реостатного торможения.", listOf("БУРТ", "ТЭД", "контакторы торможения", "вентиляция"), "Резисторные блоки и их охлаждение — по компоновке исполнения.", null, listOf("rheostatic-resistor-overheat", "rheostatic-brake", "motor-fan-failure")),
        equipment("instrument-lines", "Импульсные линии манометров", listOf("трубка манометра", "линия прибора"), "Передают давление контролируемой пневматической точки к кабинному прибору.", listOf("манометры", "ГР", "ТМ", "УР", "ТЦ"), "От пневматических точек до приборов кабины.", null, listOf("pneumatic-instrument-line-leak", "pressure-gauge-mismatch", "equalizing-reservoir-mismatch"))
    )
